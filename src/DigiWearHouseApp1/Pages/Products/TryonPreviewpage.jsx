...
const navigate=useNavigate()
  const TestFirebaseButton = () => {
    const runFirestoreTests = async () => {
      ...
      try {
        // Run the test function
        const testResult = await firebaseService.testFirestoreConnection(
          currentUser.uid
        );
  ...
  // Static model image URL
  const staticModelImage =
    "https://res.cloudinary.com/doiezptnn/image/upload/v1754996630/qgxzm85zvkchgssliiww.png";
  ...
  const calculateTotalUnits = (units, selectedSizes, selectedColors) => {
    ...
    // Only calculate for selected sizes (since colors are stored separately)
    selectedSizes.forEach((size) => {
      if (units[size]) {
        total += parseInt(units[size]) || 0;
      }
    });
  ...
  if (colorCode.includes('_')) {
    // For shade format like "pink_#FF1493", extract the base color name
    const baseName = colorCode.split('_')[0];
    return baseName.charAt(0).toUpperCase() + baseName.slice(1);
  } else {
    // For base colors like "red", just capitalize
          ...
          <div className="space-y-6">
            <div className="bg-white  rounded-lg border border-gray-200 p-6">
              ...
              <div className="flex flex-col text-start">
                <h2 className="text-xl font-semibold text-gray-900 mb-2">
                  Product Title:{productData.title || "Untitled Product"}
                </h2>
                <div className="space-y-1">
                  <p className="text-sm text-gray-600">
                    Product Type: <span className="font-semibold">{productData.chooseType || "No Category"}</span> 
                  </p>
                  <p className="text-sm text-gray-600">
                    DressType:<span className="font-semibold">  {productData.dressType || "No Category"}</span> 
                  </p>
                  <p className="text-sm text-gray-600">
                    Material Type: <span className="font-semibold">  {productData.materialType || "No Material"} </span> 
                  </p>
                  <p className="text-sm text-gray-600">
                    DesignType: <span className="font-semibold">  {productData.designType || "No Design"}</span> 
                  </p>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm text-gray-600">
                  {productData.chooseType || "No Category"}
                </div>
                <div className="text-sm text-gray-600">
                  {productData.dressType || "No Type"}
                </div>
                <div className="text-sm text-gray-600">
                  {productData.materialType || "No Material"}
                </div>
                <div className="text-sm text-gray-600">
                  Type: {productData.productType || "Ready to Wear"}
                </div>
            ...
            <div className="bg-blue-50 rounded-lg border border-blue-200 p-6">
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm text-blue-600">
                  Available Colors:{" "}
                  {productData.selectedColors?.join(", ") || "None"}
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="w-20 space-y-3">
                  ...
                  {productData.images && productData.images.length > 0
                    ...
                    : Array.from({ length: 4 }).map((_, i) => (
                        <div
                          key={i}
                          className="w-full aspect-square bg-pink-300 rounded-lg flex items-center justify-center"
                        >
                          <span className="text-white text-lg">👗</span>
                        </div>
                      ))}
